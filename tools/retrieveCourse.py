...
class NYCUTimeTableCrawler:
    ...
    def saveParseParams(self, savePath):
        self.allParams = []
        self.getType()
    ...
    def getDepartment(self, params, path:str):
        ...
        for departmentId, departmentName in departments.items():
            coursePath = f"{path}_{departmentName}"
            print(coursePath)
            self.allParams.append({"coursePath": coursePath, "params": params})

timeTableUrl = "https://timetable.nycu.edu.tw/"

nycuTimeTableCrawler = NYCUTimeTableCrawler(112, 1, 112, 1)
nycuTimeTableCrawler.saveParseParams("courseParams.json")